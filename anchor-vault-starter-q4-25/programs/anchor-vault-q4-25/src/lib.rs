...
impl<'info> Withdraw<'info> {
    pub fn withdraw(&mut self, amount: u64) -> Result<()> {
        // Ensure there are enough lamports in the vault
        ...
        // Sign with the vault PDA seeds
        let state_key = self.vault_state.key();
        let seeds: &[&[u8]] = &[b"vault", state_key.as_ref()];
        let signer_seeds: &[&[&[u8]]] = &[&[seeds[0], seeds[1], &[self.vault_state.vault_bump]]];
...
impl<'info> Close<'info> {
    pub fn close(&mut self) -> Result<()> {
        // Drain remaining lamports from the vault PDA to the user
        let vault_lamports = **self.vault.to_account_info().lamports.borrow();
        if vault_lamports > 0 {
            let cpi_program = self.system_program.to_account_info();
            let cpi_accounts = Transfer {
        ...
        // vault_state will be closed to `user` automatically because of `close = user`
        Ok(())
    }
}
