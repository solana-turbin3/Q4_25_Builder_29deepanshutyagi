...
describe("nft-staking", () => {
  // Build a provider from env or fallback to local default to avoid requiring ANCHOR_PROVIDER_URL
  ...
  const provider = new anchor.AnchorProvider(
    new anchor.web3.Connection(url, "confirmed"),
    wallet,
    { commitment: "confirmed" }
  );
  anchor.setProvider(provider);

  // Program workspace handle derived from IDL name `nftStaking` (camelCase)
